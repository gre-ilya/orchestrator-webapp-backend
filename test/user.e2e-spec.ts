...
describe('user (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    testUser = await testingMethods.findNotExistingUser(userService);
    testUser.password = 'superpass';
  ...
  it('POST /users Should return 400 (short password).', () => {
    return request(app.getHttpServer())
      .post('/users')
      .send({ email: testUser.email, password: 'shorttt' })
      .expect(400);
  });
  ...
  it('POST /users Should return 400 (wrong mail).', () => {
    return request(app.getHttpServer())
      .post('/users')
      .send({ email: 'noemail', password: testUser.password })
      .expect(400);
  });
  ...
  it('POST /users Should return 201 and { email }.', () => {
    return request(app.getHttpServer())
      ...
      .expect(201, { email: testUser.email });
  });
  ...
  it('GET /users Should return 200 and { email }.', () => {
    return request(app.getHttpServer())
      ...
      .expect(200, { email: testUser.email });
  });
  ...
  it('POST /auth/login Should return 201 (auth with new password).', async () => {
    return  request(app.getHttpServer())
        .post('/auth/login')
        .send({ email: testUser.email, password: 'newsuperpass' })
        .expect(201);
  });
  ...
  afterAll(async () => {
    try {
      await userService.remove(testUser.email);
    } catch (err) {}
    await app.close();
  });
});
